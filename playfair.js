...
var arr = [
    ...
    { "str": 'ء', "column": 0, "row": 0, "select": true, num: 36 },
...
var key = "12 32 23 11 5 6 45 1 2 6 7 19";
...
for (var i = 0; i < keyBreak.length; i++) {
...
console.log('------------------------------------');
var msg = "1 1 5 4 2 12 45 34 16 17 19";
var msgBreak = msg.split(' ');
var tempMsgBreak = [];
console.log('Message to encode :');
for (var i = 0; i < msgBreak.length; i++) {
...
for (var i = 0; i < msgBreak.length; i = i + 2) {
...
var decoMsg = '';
// Decoding message
for (var i = 0; i < decoArr.length; i = i + 2) {
    if (arr[parseInt(decoArr[i])].column == arr[parseInt(decoArr[i + 1])].column) {
        var temp = (arr[parseInt(decoArr[i])].row - 1) % 7;
        var temp1 = (arr[parseInt(decoArr[i + 1])].row - 1) % 7;
        if (temp < 0)
            temp = 7 + temp;
        if (temp1 < 0)
            temp1 = 7 + temp1;
        ...
        var temp = (arr[parseInt(decoArr[i])].column - 1) % 7;
        var temp1 = (arr[parseInt(decoArr[i + 1])].column - 1) % 7;
        if (temp < 0)
            temp = 7 + temp;
        if (temp1 < 0)
            temp1 = 7 + temp1;
